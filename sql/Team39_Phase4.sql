...
if (select count(*) from movieplay where CompanyName = (select CompanyName from theater where Manager = i_manUsername)
									and TheaterName = (select TheaterName from theater where Manager = i_manUsername)
                                    and playDate = i_movPlayDate
	) > (select Capacity from theater where Manager = i_manUsername) then
    SET @error = 'MoviesPlay Per Day cannot exceed the theater capacity.';
	SIGNAL SQLSTATE '45000' SET message_text = @error;
else
